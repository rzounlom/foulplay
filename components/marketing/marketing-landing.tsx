"use client";

import Link from "next/link";
import { ShareModal } from "@/components/game/share-modal";
import { getAppUrl } from "@/lib/host";
import { useState } from "react";

export function MarketingLanding() {
  const [shareModalOpen, setShareModalOpen] = useState(false);
  const appUrl = getAppUrl();
  const shareUrl =
    typeof window !== "undefined"
      ? window.location.href
      : "https://foulplay.io";
  const appBase = appUrl.replace(/\/$/, "");
  const createRoomUrl = `${appBase}/create`;
  const joinRoomUrl = `${appBase}/join`;

  const socialQuoteUnderCtas = (
    <div
      className="mt-3 max-w-md mx-auto text-center px-2"
      aria-label="What players say"
    >
      <p className="text-sm text-neutral-200 leading-snug">
        We were dying laughing 😂
      </p>
      <p className="mt-1 text-xs text-neutral-500 leading-snug">
        Way more fun than just watching
      </p>
    </div>
  );

  return (
    <div className="h-screen flex flex-col text-white overflow-hidden relative bg-neutral-950">
      {/* Full-viewport background - tablet/desktop only; mobile uses stacked layout */}
      <div
        className="hidden md:block absolute inset-0 bg-cover bg-center opacity-70"
        style={{ backgroundImage: "url(/social-branding.png)" }}
        aria-hidden
      />
      <div
        className="hidden md:block absolute inset-0 bg-gradient-to-b from-neutral-950/40 to-neutral-950/90"
        aria-hidden
      />
      <header className="relative z-10 shrink-0 border-b border-white/10 bg-black/20 backdrop-blur-sm">
        <div className="container mx-auto px-4 py-3 flex items-center justify-between">
          <span className="text-xl font-bold text-primary">FoulPlay</span>
          <div className="flex items-center gap-3">
            <button
              type="button"
              onClick={() => setShareModalOpen(true)}
              className="text-sm font-medium text-neutral-300 hover:text-white transition-colors inline-flex items-center gap-1.5"
              aria-label="Share"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className="h-4 w-4"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
                strokeWidth={2}
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z"
                />
              </svg>
              Share
            </button>
            <Link
              href={appUrl}
              className="text-sm font-medium text-primary hover:text-primary/80 transition-colors"
            >
              Play on Web
            </Link>
          </div>
        </div>
      </header>
      {/* Mobile: stacked layout - image at top, content below (matches app landing) */}
      <div className="flex md:hidden flex-col flex-1 min-h-0 overflow-y-auto pt-[50px] bg-neutral-950">
        <div className="flex flex-col shrink-0 w-full">
          <div
            className="w-full aspect-video bg-neutral-950 bg-contain bg-top bg-no-repeat"
            style={{ backgroundImage: "url(/social-branding.png)" }}
            aria-hidden
          />
          <div className="pt-[30px] flex flex-col items-center gap-3 px-4 pb-4">
            <h1 className="text-2xl font-extrabold tracking-tight text-white text-center px-1">
              Turn every game into chaos.
            </h1>
            <p className="text-base text-neutral-200 text-center max-w-md">
              Watch the game. Compete with your friends. Every play matters.
            </p>
            <p className="text-sm text-primary/90 text-center font-medium max-w-md">
              Perfect for tonight&apos;s game.
            </p>
            <div className="flex flex-col gap-3 w-full max-w-md">
              <Link
                href={createRoomUrl}
                className="flex-1 min-h-[52px] flex items-center justify-center py-3.5 px-7 bg-primary text-white text-base font-semibold rounded-lg text-center hover:bg-primary/95 transition-all shadow-lg shadow-primary/40 hover:shadow-primary/55 ring-2 ring-primary/25"
              >
                Start a Room
              </Link>
              <Link
                href={joinRoomUrl}
                className="flex-1 min-h-[48px] flex items-center justify-center py-3 px-6 border border-white/35 text-white/75 text-sm font-semibold rounded-lg text-center bg-transparent hover:bg-white/6 hover:text-white/90 transition-colors"
              >
                Join a Room
              </Link>
            </div>
            {socialQuoteUnderCtas}
            <p className="text-sm text-neutral-400 text-center px-2 mt-1">
              Takes 10 seconds. No downloads.
            </p>
            <p className="text-xs text-neutral-500 text-center px-2">
              Play during live NFL & NBA games with friends
            </p>
            <div className="mt-6 text-center w-full opacity-70">
              <h2 className="text-base font-semibold text-neutral-400 mb-3">
                Coming soon to mobile
              </h2>
              <div className="flex flex-wrap justify-center gap-3">
                <a
                  href="#"
                  className="inline-flex items-center gap-2 px-6 py-3 bg-neutral-800 rounded-lg opacity-60 cursor-not-allowed"
                  aria-disabled
                  title="Coming soon"
                >
                  <svg
                    className="h-8 w-8"
                    viewBox="0 0 24 24"
                    fill="currentColor"
                  >
                    <path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.81-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z" />
                  </svg>
                  <span>App Store</span>
                  <span className="text-xs">(Coming soon)</span>
                </a>
                <a
                  href="#"
                  className="inline-flex items-center gap-2 px-6 py-3 bg-neutral-800 rounded-lg opacity-60 cursor-not-allowed"
                  aria-disabled
                  title="Coming soon"
                >
                  <svg className="h-8 w-8" viewBox="0 0 24 24">
                    <path
                      fill="currentColor"
                      d="M3.609 1.814L13.792 12 3.61 22.186a.996.996 0 01-.61-.92V2.734a1 1 0 01.609-.92zm10.89 10.893l2.302 2.302-10.937 6.333 8.635-8.635zm3.199-3.198l2.807 1.626a1 1 0 010 1.73l-2.808 1.626L15.206 12l2.492-2.491zM5.864 2.658L16.802 8.99l-2.302 2.302-8.636-8.634z"
                    />
                  </svg>
                  <span>Google Play</span>
                  <span className="text-xs">(Coming soon)</span>
                </a>
              </div>
            </div>
          </div>
        </div>
      </div>

      {/* Tablet/Desktop: overlay layout - same structure as app landing page */}
      <div className="hidden md:flex relative z-10 flex-1 flex flex-col min-h-0 overflow-hidden items-center justify-center">
        <div className="flex flex-col items-center justify-center w-full max-w-4xl flex-1 min-h-0 pt-[230px] px-4 py-4">
          <div className="max-w-4xl mx-auto text-center space-y-2 sm:space-y-4 md:space-y-5">
            <div className="mx-auto w-fit max-w-full rounded-2xl border border-white/15 bg-white/10 px-5 py-3 backdrop-blur-sm sm:px-7 sm:py-3.5">
              <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-extrabold tracking-tight">
                Turn every game into chaos.
              </h1>
            </div>
            <p className="text-base md:text-lg text-neutral-300 max-w-2xl mx-auto">
              Watch the game. Compete with your friends. Every play matters.
            </p>
            <p className="text-sm md:text-base text-primary/90 font-medium">
              Perfect for tonight&apos;s game.
            </p>
            <div className="flex flex-col sm:flex-row gap-3 justify-center items-center">
              <Link
                href={createRoomUrl}
                className="inline-flex items-center justify-center min-h-[52px] px-8 py-3.5 bg-primary text-white text-base font-semibold rounded-lg hover:bg-primary/95 transition-all shadow-lg shadow-primary/40 hover:shadow-primary/55 ring-2 ring-primary/25"
              >
                Start a Room
              </Link>
              <Link
                href={joinRoomUrl}
                className="inline-flex items-center justify-center min-h-[48px] px-6 py-2.5 border border-white/35 text-white/75 text-sm font-semibold rounded-lg bg-transparent hover:bg-white/6 hover:text-white/90 transition-colors"
              >
                Join a Room
              </Link>
            </div>
            {socialQuoteUnderCtas}
            <p className="text-sm text-neutral-400 max-w-xl mx-auto">
              Takes 10 seconds. No downloads.
            </p>
            <p className="text-xs text-neutral-500 max-w-xl mx-auto">
              Play during live NFL & NBA games with friends
            </p>
          </div>
          {/* App Store badges - inline with hero */}
          <div className="mt-6 md:mt-8 text-center shrink-0 opacity-70">
            <h2 className="text-base font-semibold text-neutral-400 mb-3">
              Coming soon to mobile
            </h2>
            <div className="flex flex-wrap justify-center gap-3">
              <a
                href="#"
                className="inline-flex items-center gap-2 px-6 py-3 bg-neutral-800 rounded-lg opacity-60 cursor-not-allowed"
                aria-disabled
                title="Coming soon"
              >
                <svg
                  className="h-8 w-8"
                  viewBox="0 0 24 24"
                  fill="currentColor"
                >
                  <path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.81-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z" />
                </svg>
                <span>App Store</span>
                <span className="text-xs">(Coming soon)</span>
              </a>
              <a
                href="#"
                className="inline-flex items-center gap-2 px-6 py-3 bg-neutral-800 rounded-lg opacity-60 cursor-not-allowed"
                aria-disabled
                title="Coming soon"
              >
                <svg className="h-8 w-8" viewBox="0 0 24 24">
                  <path
                    fill="currentColor"
                    d="M3.609 1.814L13.792 12 3.61 22.186a.996.996 0 01-.61-.92V2.734a1 1 0 01.609-.92zm10.89 10.893l2.302 2.302-10.937 6.333 8.635-8.635zm3.199-3.198l2.807 1.626a1 1 0 010 1.73l-2.808 1.626L15.206 12l2.492-2.491zM5.864 2.658L16.802 8.99l-2.302 2.302-8.636-8.634z"
                  />
                </svg>
                <span>Google Play</span>
                <span className="text-xs">(Coming soon)</span>
              </a>
            </div>
          </div>
        </div>
      </div>

      <ShareModal
        isOpen={shareModalOpen}
        onClose={() => setShareModalOpen(false)}
        url={shareUrl}
        title="Share FoulPlay"
        shareText="Check out FoulPlay — real-time social card games with friends!"
      />

      {/* Footer */}
      <footer className="relative z-10 shrink-0 border-t border-white/10 bg-black/20 backdrop-blur-sm py-3">
        <div className="container mx-auto px-4 text-center text-neutral-500 text-xs">
          <p>© {new Date().getFullYear()} FoulPlay. All rights reserved.</p>
        </div>
      </footer>
    </div>
  );
}
